...
const ORDER_LABEL: Record<Order, { label: string; color: string }> = {
  before: { label: 'A → B (FIFO)', color: 'bg-emerald-500 text-white hover:bg-emerald-500' },
  after: { label: 'B → A (out of order)', color: 'bg-red-500 text-white hover:bg-red-500' },
  equal: { label: 'A = B', color: 'bg-slate-400 text-white hover:bg-slate-400' },
  missing: { label: 'Missing', color: 'bg-muted text-muted-foreground hover:bg-muted' },
}
...
export function TimeCompare({ sources, currentSourceId, onSourceChange }: TimeCompareProps) {
  ...
  const handleExport = () => {
    const exportRows = filteredRows.map(r => ({
      ...
      Order: ORDER_LABEL[r.order].label,
    }))
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center gap-2">
            ...
            <CardTitle>Time Compare</CardTitle>
          </div>
          <CardDescription>
            Compare two datetime columns row-by-row to verify FIFO order
            (e.g. <code>created_at</code> should be before <code>shipped_at</code>).
            For multi-table comparisons, use the SQL / Join tab to join tables first —
            the joined result will appear in the data source list below.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {dateLikeColumns.length < 2 ? (
            <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 rounded-md p-3">
              ...
              The selected dataset needs at least 2 datetime columns. Detected:{' '}
              {dateLikeColumns.length === 0 ? 'none' : dateLikeColumns.map(c => c.name).join(', ')}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-end gap-3">
                <div className="space-y-1">
                  <p className="text-xs font-medium text-muted-foreground">Column A (expected earlier)</p>
                ...
                <div className="space-y-1">
                  <p className="text-xs font-medium text-muted-foreground">Column B (expected later)</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <SummaryCard label="A → B (FIFO ✓)" value={summary.acc.before} total={summary.total} color="text-emerald-600" />
                <SummaryCard label="B → A (violates FIFO)" value={summary.acc.after} total={summary.total} color="text-red-600" />
                <SummaryCard label="A = B" value={summary.acc.equal} total={summary.total} color="text-slate-600" />
                <SummaryCard label="Missing" value={summary.acc.missing} total={summary.total} color="text-muted-foreground" />
              </div>

              {summary.avg !== null && (
                <div className="text-sm text-muted-foreground">
                  Average <code>{colB} − {colA}</code>:{' '}
                  <span className="font-mono text-foreground">{formatDuration(summary.avg)}</span>
                </div>
              )}

              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs text-muted-foreground">Filter rows:</span>
              ...
              <div className="rounded-lg border overflow-hidden">
                    ...
                    <TableBody>
                      {filteredRows.slice(0, 1000).map(r => (
                        <TableRow key={r.index}>
                          ...
                          <TableCell>
                            <Badge className={`${ORDER_LABEL[r.order].color} text-xs`}>{ORDER_LABEL[r.order].label}</Badge>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                ...
                {filteredRows.length > 1000 && (
                  <div className="text-xs text-muted-foreground p-2 bg-muted/40 border-t">
                    Showing first 1,000 of {filteredRows.length} rows. Use Export to get the full set.
                  </div>
                )}
              </div>
            </>
